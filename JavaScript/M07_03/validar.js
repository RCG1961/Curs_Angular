/* https://github.com/colmeye */ class Validation {
  ...
  lengthCheck(t, s, i) {
    return t.val().length <= s
      ? "Tiene que tener como mínimo " + ++s + " carácteres. "
      : t.val().length >= i
      ? "El número máximo de carácteres es de  " + i 
      : "";
  }
  ...
  numberCheck(t) {
    return t.val().match(/\d/) ? "" : "Debe contener un número. ";
  }
  specialCharCheck(t) {
    return t.val().match(/\W|_/g) ? "" : "Debe contener un carácter especial. ";
  }
  capitalCheck(t) {
    return t.val().match(/[A-Z]+/) ? "" : "Debe contener al menos una mayúscula ";
  }
